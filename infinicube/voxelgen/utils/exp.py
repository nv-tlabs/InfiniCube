...
def parse_config_yaml(
    yaml_path: Path,
    args: Union[argparse.Namespace, OmegaConf] = None,
    override: bool = True,
    additional_includes: list = None,
) -> OmegaConf:
    if args is None:
    ...
    if "assign" in configs:
        ...
        del configs["assign"]
        assign_config = OmegaConf.from_dotlist(
...
class ArgumentParserX(ArgumentParser):
    def __init__(
        self, base_config_path=None, add_hyper_arg=True, to_oconf=True, **kwargs
    ):
        ...
        self.add_argument(
            "--exec", type=str, nargs="+", help="Extract code to modify the args"
        )
        self.add_argument(
            "--include",
            type=str,
            nargs="+",
            help="Additional configs to include, will be appended to the include_configs key",
        )
    ...
    def parse_args(self, args=None, namespace=None, additional_args=None):
        ...
        if self.add_hyper_arg and _args.hyper != "none":
            if _args.hyper.endswith("json"):
                ...
                file_args = parse_config_yaml(
                    Path(_args.hyper), file_args, additional_includes=_args.include
                )
...
class TorchLossMeter:
    ...
    def add_loss(self, name, loss, weight=1.0):
        ...
        if hasattr(loss, "numel"):
            assert loss.numel() == 1, (
                f"Loss must contains only one item, instead of {loss.numel()}."
            )
...
class AverageMeter:
    ...
    def get_printable_newest(self):
        ...
        return ", ".join(
            [
                f"{loss_name}[{nloss_count[loss_name] - 1}]: {nloss_val[loss_name]}"
        ...
        for loss_name, loss_val in losses.items():
            ...
            if loss_name not in self.loss_dict.keys():
                ...
                self.loss_dict[loss_name] = (
                    self.alpha * old_mean + (1 - self.alpha) * loss_val
                )
        ...
        if self.enabled:
            ...
            if not merged:
                for time_i in range(len(self.time_names) - 1):
                    print(
                        f"{self.time_names[time_i]} --> {self.time_names[time_i + 1]}: "
                ...
                for time_i in range(len(self.time_names) - 1):
                    ...
                    interval_time = (
                        self.time_points[time_i + 1] - self.time_points[time_i]
                    )
...
def performance_counter(n_iter: int = 1, run_ahead: int = 0, desc: str = None):
    ...
    for i_iter in range(n_iter):
        try:
            ...
            print(e)
    print(
        f" + Perf Success {len(all_times)} / {n_iter}, "
        ...
        def new_repr(self):
            ...
            for func_name in recorded_func_names:
                ...
                if peak_mem - start_mem < mem_threshold * 1024 * 1024:
                    continue
                func_info.append(
                    f"{func_name} +{readable_size(end_mem - start_mem)} "
...
def deterministic_hash(data):
    ...
    jval = json.dumps(
        data, ensure_ascii=False, sort_keys=True, indent=None, separators=(",", ":")
    )
...
class ComputeDevice:
    ...
    def __repr__(self):
        return (
            f"{self.server_name}-GPU-{self.gpu_id}: Mem: {self.gpu_mem_usage * 100:.2f}%, "
...
def get_gpu_status(server_name, get_process_info: bool = False, use_nvml: bool = True):
    ...
    if get_process_info:
        cwd_output = run_command(
            ["ssh", server_name, "pwdx", " ".join([t[1] for t in proc_info])]
        )
