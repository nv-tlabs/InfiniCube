...
import wandb
from infinicube.voxelgen.utils.exp import ConsoleColor
...
def get_wandb_run(
    wdb_url: str, wdb_base: str = "./wandb/", default_ckpt: str = "all", is_train=True
):
    """
    ...
    if (
        len(other_name) == 8
        and "/" not in other_name
        and "_" not in other_name
        and "-" not in other_name
    ):
        ...
        all_runs = wandb.Api().runs(
            f"{user_name}/{project_name}", filters={"display_name": other_name}
        )
    ...
    if ckpt_name == "last":
        ...
        if is_train:
            try:
                ...
                print(
                    f'{ConsoleColor.YELLOW} Warning: The existing "last" checkpoint is incomplete, '
        ...
        try:
            last_mtime = (
                wdb_run.history(keys=["val_loss", "_timestamp"], samples=1000)
                .iloc[-1]
                ._timestamp
            )
            if last_mtime - existing_mtime > 600:  # 10min threshold
                print(
                    f'{ConsoleColor.YELLOW} Warning: The existing "last" checkpoint has timestamp '
        ...
        if best_epoch_idx in epoch_ckpts:
            ...
            print(
                f"{ConsoleColor.YELLOW} Best epoch is {best_epoch_idx} but not found, existed: {list(epoch_ckpts.keys())} "
            )
            print(loss_table)
            print("")
        ...
        if len(epoch_ckpts) == 0:
            print(
                f"{ConsoleColor.YELLOW} Please check the folder of {ckpt_base}, {list(ckpt_base.glob('*.ckpt'))} {ConsoleColor.RESET}"
            )
            raise FileNotFoundError
        ...
        if existing_last_epoch_idx == best_epoch_idx == last_epoch_idx:
            print(
                f"{ConsoleColor.YELLOW} Warning: ckpt not specified, but {last_epoch_idx} (being both last and best) "
                f"seems to be a valid choice. {ConsoleColor.RESET}"
            )
            ckpt_path = epoch_ckpts[existing_last_epoch_idx]
        else:
            print(
                f"{ConsoleColor.YELLOW} Cannot determine correct ckpt_idx! Best = {best_epoch_idx}, Last = {last_epoch_idx}, "
        ...
        if epoch_idx in epoch_ckpts:
            ...
            print(
                f"{ConsoleColor.YELLOW} Epoch {epoch_idx} not found, existed: {list(epoch_ckpts.keys())} {ConsoleColor.RESET}"
            )
            raise FileNotFoundError
