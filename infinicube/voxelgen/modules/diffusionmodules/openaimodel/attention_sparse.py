...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, glu=False, dropout=0.0):
        ...
        project_in = (
            nn.Sequential(fvnn.Linear(dim, inner_dim), GELU())
            if not glu
            else GEGLU(dim, inner_dim)
        )

        self.net = nn.Sequential(
            project_in, fvnn.Dropout(dropout), fvnn.Linear(inner_dim, dim_out)
        )
...
class Attention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0):
        ...
        self.to_out = nn.Sequential(
            fvnn.Linear(inner_dim, query_dim), fvnn.Dropout(dropout)
        )
    ...
    def forward(self, x: VDBTensor):
        ...
        for batch_idx in range(q.grid.grid_count):
            out.append(
                self._attention(
                    q.data[batch_idx].jdata,
                    k.data[batch_idx].jdata,
                    v.data[batch_idx].jdata,
                )
            )
    ...
    def _attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        h = self.heads
        q, k, v = map(
            lambda t: rearrange(t, "(b n) (h d) -> b h n d", h=h, b=1), (q, k, v)
        )
            ...
            if exists(mask):
                ...
                out.append(
                    self._attention(
                        ...
                        k[batch_idx],
                        v[batch_idx],
                        mask=mask[batch_idx : batch_idx + 1],
                    )
                )
            else:
                out.append(
                    self._attention(q.data[batch_idx].jdata, k[batch_idx], v[batch_idx])
                )
        out = fvdb.JaggedTensor(out)
        out = VDBTensor(x.grid, out, x.kmap)
        return self.to_out(out)
