...
from infinicube.voxelgen.modules.diffusionmodules.openaimodel.attention_sparse import (
    SpatialTransformer,
)
...
class TimestepSequential(nn.Sequential):
    def forward(
        self, x, emb, target_tensor: Optional[VDBTensor] = None, context=None, mask=None
    ):
        for layer in self:
...
class UNetModel(nn.Module):
    ...
    def forward(
        ...
        for block in self.decoder_blocks:
            ...
            data = block(
                data, emb, hs[-1] if len(hs) > 0 else None, context=context, mask=mask
            )
