...
class ResBlock(TimestepModule):
    def __init__(
        ...
        self.emb_layers = nn.Sequential(
            nn.SiLU(), nn.Linear(self.emb_channels, 2 * self.out_channels)
        )
        ...
        self.out_layers = nn.Sequential(
            ...
            zero_module(
                fvnn.SparseConv3d(self.out_channels, self.out_channels, 3, bias=True)
            ),
        )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = fvnn.SparseConv3d(
                channels, self.out_channels, 1, bias=True
            )
    ...
    def forward(
        ...
        if self.up or self.down:
            ...
            data_h = in_rest(data)
            data_h = (
                self.up_module(data_h, ref_fine_data=target_tensor)
                if self.up
                else self.down_module(data_h, ref_coarse_data=target_tensor)
            )
            data_h = in_conv(data_h)
            data = (
                self.up_module(data, ref_fine_data=data_h)
                if self.up
                else self.down_module(data, ref_coarse_data=data_h)
            )
        else:
        ...
        if data.grid.grid_count == 1:
            # RuntimeError: batch indices' shape should match between this tensor and other tensor
            out_jagged_tensor = JaggedTensor(data_h.data.jdata) + JaggedTensor(
                self.skip_connection(data).data.jdata
            )
            data = VDBTensor(data.grid, out_jagged_tensor)
        else:
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x: VDBTensor):
        ...
        if x.grid.grid_count == 1:
            # RuntimeError: batch indices' shape should match between this tensor and other tensor
            out_jagged_tensor = JaggedTensor(feature.data.jdata) + JaggedTensor(
                x.data.jdata
            )
            out_vdb_tensor = VDBTensor(x.grid, out_jagged_tensor)
        else:
            out_vdb_tensor = feature + x

        return out_vdb_tensor
...
class UNetModel(nn.Module):
        ...
        **kwargs,
        ...
        self.encoder_blocks = nn.ModuleList(
            [
                TimestepSequential(
                    fvnn.SparseConv3d(
                        self.in_channels, self.model_channels, 3, bias=True
                    ),
                )
            ]
        )
        ...
        self.out_block = nn.Sequential(
            ...
            fvnn.SiLU(),
            zero_module(
                fvnn.SparseConv3d(current_channels, self.out_channels, 3, bias=True)
            ),
        )
    ...
    def forward(
        self, data: VDBTensor, timesteps: torch.Tensor, y: Optional[torch.Tensor] = None
    ):
