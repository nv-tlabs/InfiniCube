...
class DepthAnythingFeatureFusionStage(nn.Module):
    ...
    def forward(self, hidden_states, size=None):
        ...
        for idx, (hidden_state, layer) in enumerate(
            zip(hidden_states[1:], self.layers[1:])
        ):
            size = (
...
class DepthAnythingForDepthEstimation(DepthAnythingPreTrainedModel):
    ...
    def forward_fusion(
        ...
        """
        if depth anything v2 large. pixel_values: [N, 3, p_h * 14, p_w * 14]

        Returns:
            hidden_states: list of 4 elements
                hidden_states[0]: [N, 256, p_h, p_w]
                hidden_states[1]: [N, 256, p_h * 2, p_w * 2]
                hidden_states[2]: [N, 256, p_h * 4, p_w * 4]
                hidden_states[3]: [N, 256, p_h * 8, p_w * 8]
        """
        loss = None
        if labels is not None:
            raise NotImplementedError("Training is not implemented yet")
