...
class KitchenSinkMetricLoss:
    ...
    def _get_svh_samples(
        ...
        local_ids = (
            torch.rand((n_samples,), device=svh.device) * base_coords.jdata.size(0)
        ).long()
        local_coords = (
            torch.rand((n_samples, 3), device=svh.device) - 0.5
        ) * base_scales.jdata[local_ids]
        ...
        return JaggedTensor.from_data_and_jidx(
            query_pos, query_jidx, svh.grids[-1].grid_count
        )
    ...
    def _get_samples(cls, hparams, configs, svh, ref_xyz, ref_normal):
        ...
        for config in configs:
            if config.type == "uniform":
                all_samples.append(
                    cls._get_svh_samples(
                        svh, config.n_samples, config.expand, config.expand_top
                    )
                )
            elif config.type == "band":
                band_inds = (
                    torch.rand((config.n_samples,), device=ref_xyz.device)
                    * ref_xyz.jdata.size(0)
                ).long()
                sample_jidx = ref_xyz.jidx[band_inds]
                eps = config.eps * hparams.voxel_size
                band_pos = (
                    ref_xyz.jdata[band_inds]
                    + ref_normal.jdata[band_inds]
                    * torch.randn((config.n_samples, 1), device=ref_xyz.jdata.device)
                    * eps
                )
                all_samples.append(
                    JaggedTensor.from_data_and_jidx(
                        band_pos, sample_jidx, svh.grids[-1].grid_count
                    )
                )
    ...
    @classmethod
    def compute_gt_chi_from_pts(
        cls,
        hparams,
        query_pos: JaggedTensor,
        ref_xyz: JaggedTensor,
        ref_normal: JaggedTensor,
        transform: bool = True,
    ):
        mc_query_sdfs = []
        for b in range(query_pos.joffsets.size(0)):
            q_pos, r_xyz, r_normal = (
                query_pos[b].jdata,
                ref_xyz[b].jdata,
                ref_normal[b].jdata,
            )
            stdv = 0.2 if hparams.voxel_size > 0.004 else 0.002
            mc_query_sdfs.append(
                -ext.sdfgen.sdf_from_points(q_pos, r_xyz, r_normal, 8, stdv, False)[0]
            )
...
class UDFLoss(KitchenSinkMetricLoss):
    ...
    def compute_gt_tudf(cls, chi_pos, hparams, ref_xyz, ref_normal, ref_geometry):
        if ref_geometry is not None:
            gt_tsdf = cls.transform_field(
                hparams, JaggedTensor([ref_geometry.query_sdf(chi_pos.jdata)])
            )
        else:
    ...
    def apply(cls, hparams, loss_dict, metric_dict, batch, out, compute_metric):
        ...
        if hparams.udf.enabled and udf_config.weight > 0.0:
            if DS.GT_GEOMETRY not in batch.keys():
                ref_geometry = None
                ref_xyz, ref_normal = (
                    JaggedTensor(batch[DS.GT_DENSE_PC]),
                    JaggedTensor(batch[DS.GT_DENSE_NORMAL]),
                )
            else:
                assert len(batch[DS.GT_GEOMETRY]) == 1, (
                    "Only supports single batch for now"
                )
                ref_geometry = batch[DS.GT_GEOMETRY][0]
                ref_xyz, ref_normal, _ = ref_geometry.torch_attr()
                ref_xyz, ref_normal = (
                    JaggedTensor([ref_xyz]),
                    JaggedTensor([ref_normal]),
                )
            ...
            chi_pos = cls._get_samples(
                hparams, udf_config.samplers, field_svh, ref_xyz, ref_normal
            )
            pd_chi = udf_field.evaluate_f(chi_pos).value

            gt_tudf = cls.compute_gt_tudf(
                chi_pos, hparams, ref_xyz, ref_normal, ref_geometry
            )
            pd_tudf = cls.transform_field(hparams, pd_chi).jdata
            udf_loss_normalized = torch.mean(
                torch.abs(pd_tudf - gt_tudf) / hparams.voxel_size
            )

            loss_dict.add_loss("udf", udf_loss_normalized, udf_config.weight)
        ...
        if hparams.supervision.structure_weight > 0.0:
            ...
            for feat_depth, struct_feat in out["structure_features"].items():
                if struct_feat.jdata.size(0) == 0:
                    continue
                gt_status = gt_svh.evaluate_voxel_status(
                    out["dec_tmp_svh"].grids[feat_depth], feat_depth
                )
                loss_dict.add_loss(
                    f"struct-{feat_depth}",
                    F.cross_entropy(struct_feat.jdata, gt_status),
                    hparams.supervision.structure_weight,
                )
                if compute_metric:
                    metric_dict.add_loss(
                        f"struct-acc-{feat_depth}",
                        torch.mean(
                            (struct_feat.jdata.argmax(dim=1) == gt_status).float()
                        ),
                    )
    ...
    def apply(cls, hparams, loss_dict, metric_dict, batch, out, compute_metric):
        ...
        if gt_surface_config.value > 0.0 or gt_surface_config.normal > 0.0:
            if DS.GT_GEOMETRY not in batch.keys():
                ...
                ref_xyz, ref_normal = (
                    JaggedTensor([t[0] for t in ref_xyz_normal]),
            ...
            if 0 < n_subsample < ref_xyz.jdata.size(0):
                ref_xyz_inds = (
                    torch.rand((n_subsample,), device=ref_xyz.device)
                    * ref_xyz.jdata.size(0)
                ).long()
            else:
                ref_xyz_inds = torch.arange(
                    ref_xyz.jdata.size(0), device=ref_xyz.device
                )
            ...
            ref_xyz = JaggedTensor.from_data_and_jidx(
                ref_xyz.jdata[ref_xyz_inds], ref_jidx, batch_size
            )

            eval_res = field.evaluate_f(ref_xyz, grad=compute_grad)

            if compute_grad:
                ref_normal = JaggedTensor.from_data_and_jidx(
                    ref_normal.jdata[ref_xyz_inds], ref_jidx, batch_size
                )
                pd_grad = eval_res.gradient.jdata
                pd_grad = -pd_grad / (
                    torch.linalg.norm(pd_grad, dim=-1, keepdim=True) + 1.0e-6
                )
                loss_dict.add_loss(
                    "gt-surface-normal",
            ...
            loss_dict.add_loss(
                "gt-surface-value",
                torch.abs(eval_res.value.jdata).mean(),
                gt_surface_config.value,
            )
...
class SpatialLoss(KitchenSinkMetricLoss):
    ...
    def apply(cls, hparams, loss_dict, metric_dict, batch, out, compute_metric):
        ...
        if opt.weight > 0.0:
            chi_pos = cls._get_samples(
                hparams, opt.samplers, field.svh, ref_xyz, ref_normal
            )
            ...
            if ref_geometry is not None:
                ...
                gt_tsdf = cls.compute_gt_chi_from_pts(
                    hparams, chi_pos, ref_xyz, ref_normal
                )

                near_surface_mask = torch.ones(
                    chi_pos.jdata.size(0), dtype=bool, device=chi_pos.device
                )
                empty_space_mask = ~near_surface_mask

            pd_tsdf = cls.transform_field(hparams, pd_chi)
            near_surface_l1 = torch.abs(
                (pd_tsdf.jdata[near_surface_mask] - gt_tsdf.jdata[near_surface_mask])
                / hparams.voxel_size
            )

            # Empty space: value as small as possible.
            empty_scale = 2.0 * hparams.voxel_size
            empty_space_loss = 0.1 * torch.exp(
                pd_chi.jdata[empty_space_mask] / empty_scale
            )
            mixed_loss = (
                torch.sum(near_surface_l1) + torch.sum(empty_space_loss)
            ) / chi_pos.jdata.size(0)
            loss_dict.add_loss("spatial", mixed_loss, opt.weight)

            # RegSDF Loss:
            if opt.reg_sdf_weight > 0.0:
                reg_sdf_eps = 0.5
                reg_sdf_loss = torch.mean(
                    reg_sdf_eps / (pd_chi.jdata**2 + reg_sdf_eps**2)
                )
                loss_dict.add_loss("msa", reg_sdf_loss, opt.reg_sdf_weight)
