...
from infinicube.voxelgen.utils.color_util import color_from_points, semantic_from_points
from infinicube.voxelgen.utils.loss_util import TorchLossMeter
...
class Loss(nn.Module):
    ...
    def cross_entropy(
        self,
        pd_struct: fvnn.VDBTensor,
        gt_grid: fvdb.GridBatch,
        dynamic_grid: fvdb.GridBatch = None,
    ):
        assert torch.allclose(pd_struct.grid.origins, gt_grid.origins, atol=1e-7), (
            f"{pd_struct.grid.origins} != {gt_grid.origins}"
        )
        assert torch.allclose(
            pd_struct.grid.voxel_sizes, gt_grid.voxel_sizes, atol=1e-7
        ), f"{pd_struct.grid.voxel_sizes} != {gt_grid.voxel_sizes}"
        idx_mask = (
            gt_grid.ijk_to_index(pd_struct.grid.ijk).jdata == -1
        )  # 0 is occupied, 1 is empty
        idx_mask = idx_mask.long()

        if self.hparams.supervision.adaptive_structure_weight:
            # min(max(#N/#P, 1), 100)
            positive_weight = min(
                max(1, idx_mask.shape[0] / (torch.sum(idx_mask == 0) + 1e-6).item()),
                100,
            )
            # weight for F.cross_entropy is a 1D tensor, setting weight for each class
            weight = torch.Tensor([positive_weight, 1.0]).to(idx_mask.device)
        else:
            weight = None
        ...
        if dynamic_grid is not None:
            ...
            loss = (
                F.cross_entropy(
                    pd_struct.data.jdata, idx_mask, weight=weight, reduction="none"
                )
                * dynamic_mask.float()
            )
            loss = loss.mean()
        else:
            loss = F.cross_entropy(pd_struct.data.jdata, idx_mask, weight=weight)
    ...
    def struct_acc(self, pd_struct: fvnn.VDBTensor, gt_grid: fvdb.GridBatch):
        ...
        idx_mask = idx_mask.long()
        return torch.mean((pd_struct.data.jdata.argmax(dim=1) == idx_mask).float())
    ...
    def color_loss(self, batch, color_feats: fvnn.VDBTensor):
        ...
        for batch_idx in range(ref_grid.grid_count):
            ...
            target_color.append(
                color_from_points(
                    target_xyz[batch_idx].jdata,
                    ref_xyz[batch_idx].jdata,
                    ref_color_i,
                    k=1,
                )
            )
            slect_color_feats.append(color_feats.data[batch_idx].jdata)

        if (
            len(target_color) == 0 or len(slect_color_feats) == 0
        ):  # to avoid JaggedTensor build from empty list
            return 0.0
    ...
    def semantic_loss(self, batch, semantic_feats: fvnn.VDBTensor):
        ...
        if (
            ref_semantic.jdata.size(0) == 0
        ):  # if all samples in this batch is without semantic
            return 0.0
        ...
        for batch_idx in range(ref_grid.grid_count):
            ...
            slect_semantic_feats.append(semantic_feats.data[batch_idx].jdata)

        if (
            len(target_semantic) == 0 or len(slect_semantic_feats) == 0
        ):  # to avoid JaggedTensor build from empty list
            return 0.0
        ...
        if (
            slect_semantic_feats.jdata.size(0) == 0
        ):  # to aviod cross_entropy take empty tensor
            return 0.0

        semantic_loss = F.cross_entropy(
            slect_semantic_feats.jdata, target_semantic.jdata.long()
        )
                ...
                for feat_depth, pd_struct_i in out["structure_features"].items():
                    ...
                    if self.hparams.remain_h:
                        ...
                        downsample_factor = [
                            downsample_factor,
                            downsample_factor,
                            downsample_factor // h_factor,
                        ]
                    if downsample_factor != 1:
                        gt_grid_i = gt_grid.coarsened_grid(downsample_factor)
                        dyn_grid_i = (
                            dynamic_grid.coarsened_grid(downsample_factor)
                            if dynamic_grid is not None
                            else None
                        )
                    else:
                    ...
                    loss_dict.add_loss(
                        f"struct-{feat_depth}",
                        self.cross_entropy(pd_struct_i, gt_grid_i, dyn_grid_i),
                    ...
                    if compute_metric:
                        with torch.no_grad():
                            metric_dict.add_loss(
                                ...
                                self.struct_acc(pd_struct_i, gt_grid_i),
                            )
        else:
            if self.hparams.supervision.structure_weight > 0.0:
                gt_tree = out["gt_tree"]
                for feat_depth, pd_struct_i in out["structure_features"].items():
                    ...
                    # get dynamic grid
                    dyn_grid_i = (
                        dynamic_grid.coarsened_grid(2**feat_depth)
                        if dynamic_grid is not None
                        else None
                    )
                    loss_dict.add_loss(
        ...
        if self.hparams.with_normal_branch:
            if out["normal_features"] == {}:
                ...
                normal_loss = self.normal_loss(
                    batch, out["normal_features"][feat_depth]
                )

            loss_dict.add_loss(
                "normal", normal_loss, self.hparams.supervision.normal_weight
            )
        ...
        if self.hparams.with_semantic_branch:
            for feat_depth, pd_semantic_i in out["semantic_features"].items():
                ...
                if semantic_loss == 0.0:  # do not take empty into log
                    continue
                loss_dict.add_loss(
                    f"semantic_{feat_depth}",
                    semantic_loss,
                    self.hparams.supervision.semantic_weight,
                )
        ...
        if self.hparams.with_color_branch:
            for feat_depth, pd_color_i in out["color_features"].items():
                ...
                loss_dict.add_loss(
                    f"color_{feat_depth}",
                    color_loss,
                    self.hparams.supervision.color_weight,
                )
