...
class Encoder(nn.Module):
    def __init__(self, hparams):
        ...
        if self.hparams.use_input_semantic:
            ...
            self.semantic_embed_fn = nn.Embedding(
                self.hparams.num_semantic, self.hparams.dim_semantic
            )
    ...
    def forward(self, grid: GridBatch, batch) -> torch.Tensor:
        input_normal = (
            batch[DS.TARGET_NORMAL] if DS.TARGET_NORMAL in batch.keys() else None
        )
        if self.hparams.use_input_color:
        ...
        if self.hparams.use_input_normal:
            ...
            # splatting normal
            input_normal = grid.splat_trilinear(
                ref_xyz, fvdb.JaggedTensor(input_normal)
            )
            # normalize normal
