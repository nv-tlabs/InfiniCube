...
class CustomizedDataParallel(DataParallel):
    def scatter(self, inputs, kwargs, device_ids):
        inputs = (
            self.module.module.dp_scatter(inputs, device_ids, self.dim)
            if inputs
            else []
        )
        kwargs = (
            self.module.module.dp_scatter(kwargs, device_ids, self.dim)
            if kwargs
            else []
        )
...
class CustomizedDataParallelPlugin(DataParallelPlugin):
    ...
    def setup(self, model):
        ...
        self._model = CustomizedDataParallel(
...
def determine_usable_gpus():
    ...
    available_gpus = [
        t for t in gpu_states if t.gpu_mem_usage < 0.2 and t.gpu_compute_usage < 0.2
    ]
    ...
    if len(available_gpus) < program_args.gpus:
        print(
            f"Warning: Available GPUs are {[t.gpu_id for t in available_gpus]}, "
...
def readable_name_from_exec(exec_list: List[str]):
    ...
    for exec_str in exec_list:
        ...
        k_name = kvs[0]
        k_name_arr = [
            "".join([us[0] for us in t.split("_") if len(us) > 0])
            for t in k_name.split(".")
        ]
...
if __name__ == "__main__":
    ...
    program_parser.add_argument(
        "--debug", action="store_true", help="Use debug mode of pytorch"
    )
    program_parser.add_argument(
        "--resume",
        action="store_true",
        help="Continue training. Use hparams.yaml file.",
    )
    program_parser.add_argument(
        "--nolog", action="store_true", help="Do not create any logs."
    )
    program_parser.add_argument(
    ...
    program_parser.add_argument(
        "--logger_type", choices=["tb", "wandb", "none"], default="wandb"
    )
    program_parser.add_argument(
        "--wname", default=None, type=str, help="Run name to be appended to wandb"
    )
    program_parser.add_argument(
        "--wandb_base",
        type=str,
    ...
    program_parser.add_argument(
        "--eval_interval", type=int, default=1, help="How often to evaluate the model."
    )
    program_parser.add_argument(
        "--save_every", default=50, type=int, help="How often to save the model."
    )
    program_parser.add_argument(
        "--resume_from_ckpt",
        default=None,
        type=str,
        help="checkpoint path we want to load",
    )
    program_parser.add_argument(
        "--model_precision", default=32, help="Model precision to use."
    )
    program_parser.add_argument(
        "--seed", type=int, default=0, help="Set a random seed."
    )
    ...
    model_parser = exp.ArgumentParserX(
    ...
    if program_args.logger_type == "wandb":
        ...
        # replace nvidia-toronto to your own wandb account organization name
        check_wandb_name = "wdb:nvidia-toronto/infinicube-%s/%s:last" % (
            project_name,
            run_name,
        )
        ...
        try:
            ...
            wdb_run, args_ckpt = wandb_util.get_wandb_run(
                check_wandb_name, wdb_base=program_args.wandb_base, default_ckpt="last"
            )
    ...
    if program_args.resume:
        ...
        if raw_hyper.startswith("wdb:"):
            ...
            wdb_run, wdb_ckpt = wandb_util.get_wandb_run(
                raw_hyper, program_args.wandb_base, default_ckpt="last"
            )
    ...
    if program_args.logger_type == "tb":
        if not program_args.resume:
            ...
            logger_version_num = (
                Path(hyper_path).parent.name if program_args.resume else None
            )
        logger = TensorBoardLogger(
            "../checkpoints/",
            name=model_args.name,
            version=logger_version_num,
            default_hp_metric=False,
        )
        # Call this property to assign the version early, so we don't have to wait for the model to be loaded
        print("Tensorboard logger, version number =", logger.version)
        ...
        if not program_args.resume:
            ...
            if "WANDB_SWEEP_ID" in os.environ.keys():
                ...
                wname = (
                    os.environ["WANDB_SWEEP_ID"]
                    + "-"
                    + readable_name_from_exec(model_args.exec)
                )
            if wname is None:
                # Example: 0105-clever-monkey
                wname = (
                    (datetime.utcnow() + timedelta(hours=8)).strftime("%m%d")
                    + "-"
                    + randomname.get_name()
                )
            sep_pos = str(model_args.name).find("/")
            ...
            if is_rank_node_zero():
                logger = WandbLogger(
                    name=run_name,
                    save_dir=program_args.wandb_base,
                    project="infinicube-" + project_name,
                )
            else:
                pass
        else:
    ...
    if trainer.current_epoch < 1 and last_ckpt_path is None and trainer.local_rank == 0:
        if program_args.logger_type == "tb":
            ...
            with open(".premature_checkpoints", "a") as f:
                f.write(
                    f"wdb:{trainer.logger.experiment.path}:{trainer.logger.experiment.name}\n"
                )
            print(
