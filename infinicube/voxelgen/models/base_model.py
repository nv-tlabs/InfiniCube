...
from infinicube.voxelgen.utils.loss_util import AverageMeter
from infinicube.voxelgen.utils.voxel_util import create_fvdb_grid_w_semantic_from_points
...
class BaseModel(pl.LightningModule):
    ...
    def load_module(spec_path, weight_path=None, overwrite_config=None):
        ...
        if spec_path is not None:
            ...
            config_args = exp.parse_config_yaml(
                config_yaml_path, overwrite_config, override=False
            )
        else:
        ...
        basis_net_module = importlib.import_module(
            "infinicube.voxelgen.models." + config_args.model
        ).Model

        if weight_path is not None:
            net_module = basis_net_module.load_from_checkpoint(
                weight_path, hparams=config_args
            )
        else:
    ...
    def training_step(self, *args, **kwargs):
        try:
            ...
            exp.logger.warning("Training-step fail. Skipping.")

            try:
                from infinicube.voxelgen.data.base import DatasetSpec as DS

                exp.logger.warning(
                    f"The problematic batch is: {args[0][DS.SHAPE_NAME]}"
                )
            except:
                pass
            ...
            self.num_fail += 1.0
            self.log("num_fail", self.num_fail)
            return None
    ...
    def validation_step(self, *args, **kwargs):
        try:
            ...
            # Compare to post-mortem, this would allow training to continue...
            exp.logger.warning("Validation-step fail. Skipping.")
            traceback.print_exc()
            gc.collect()
            torch.cuda.empty_cache()
            self.num_fail += 1.0
    ...
    def on_after_backward(self):
        ...
        if exp.global_var_manager.get("skip_backward"):
            exp.logger.info(
                "Skip backward is enabled. This step won't affect the model."
            )
        ...
        if has_nan_value_cnt > 0:
            exp.logger.warning(
                f"{has_nan_value_cnt} parameters get nan-gradient -- this step will be skipped."
            )
            for p in filter(lambda p: p.grad is not None, self.parameters()):
                p.grad.data.zero_()
        ...
        if not self.trainer.sanity_checking:
            ...
            for metric_name, should_add_best in self.get_hparams_metrics():
                if should_add_best and metric_name in logged_metrics.keys():
                    self.best_metrics.append_loss(
                        {metric_name: logged_metrics[metric_name].item()}
                    )
                    ...
                    self.log(
                        f"best/{metric_name}",
                        np.min(self.best_metrics.loss_dict[metric_name]),
                    )
    ...
    def log_dict_prefix(
        ...
        """
        dictionary = {prefix + "/" + str(k): v for k, v in dictionary.items()}
        self.log_dict(
        ...
        if self.trainer.logger is not None:
            if self.logger_type == "tb":
                ...
                self.trainer.logger.experiment.add_image(
                    name, img, self.trainer.global_step
                )
                ...
                self.trainer.logger.log_image(
                    key=name, images=img, file_type=["jpg"] * len(img)
                )
    ...
    def on_test_start(self):
        if self.hparams.get("record", None) is not None:
            ...
            if len(self.hparams.record) == 0:
                model_name = (
                    inspect.getfile(self.__class__).split("/")[-1].split(".py")[0]
                )
            ...
            shutil.copy(
                f"models/{self.hparams.model.replace('.', '/')}.py",
                self.record_folder / "model.py",
            )
    ...
    def print_test_logs(self):
        ...
        for h in headers:
            ...
            print(
                h,
                f"({len(header_values)})",
                np.mean(header_values)
                if isinstance(header_values[0], float)
                else "NOT-NUMERIC",
            )
    ...
    def on_test_batch_end(self, outputs, batch, batch_idx, dataloader_idx):
        ...
        if self.record_folder is not None:
            # If has valid log
            if (
                len(self.test_logged_values) > 0
                and self.test_logged_values[-1]["batch-idx"] == batch_idx
            ):
            ...
            if len(self.record_data_cache) > 0:
                pkl_data = {
                    k: v
                    for k, v in self.record_data_cache.items()
                    if type(v) not in self.TEST_SPECIAL_META.keys()
                }
                special_data = {
                    k: v
                    for k, v in self.record_data_cache.items()
                    if k not in pkl_data.keys()
                }
                ...
                for k, v in special_data.items():
                    ...
                    write_f(
                        str(self.record_folder / k / f"{batch_idx:06d}.{suffix}"), v
                    )

                if len(pkl_data) > 0:
                    (self.record_folder / "test_log_data").mkdir(
                    ...
                    with (
                        self.record_folder / "test_log_data" / f"{batch_idx:06d}.pkl"
                    ).open("wb") as f:
                        pickle.dump(res, f)
    ...
    @staticmethod
    def generate_fvdb_grid_on_the_fly(batch):
        if (
            DS.INPUT_PC_RAW in batch
            and DS.INPUT_PC in batch
            and not isinstance(batch[DS.INPUT_PC], fvdb.GridBatch)
        ):
            B = len(batch[DS.INPUT_PC])

            generated_grids, generated_semantics = (
                create_fvdb_grid_w_semantic_from_points(
                    batch[DS.INPUT_PC_RAW]["points_finest"],
                    batch[DS.INPUT_PC_RAW]["semantics_finest"],
                    {
                        "voxel_sizes": batch[DS.INPUT_PC_RAW][
                            "grid_batch_kwargs_target"
                        ]["voxel_sizes"][0],
                        "origins": batch[DS.INPUT_PC_RAW]["grid_batch_kwargs_target"][
                            "origins"
                        ][0],
                    },
                    {
                        ...
                        "origins": batch[DS.INPUT_PC_RAW]["grid_batch_kwargs_finest"][
                            "origins"
                        ][0],
                    },
            ...
            batch[DS.INPUT_PC] = fvdb.jcat(generated_grids)
    ...
    def generate_latent_semantic_on_the_fly(batch, latent_grid):
        """
        For semantic sparse diffusion in waymo_wds, we construct semantic grid from INPUT_PC for the latent_vdb_tensor,
        which is usually a 4x/8x downsampled grid from the VAE encoder
        ...
        for i in range(B):
            latent_xyz = latent_grid[i].grid_to_world(latent_grid[i].ijk.float()).jdata
            input_pc_xyz = (
                batch[DS.INPUT_PC][i]
                .grid_to_world(batch[DS.INPUT_PC][i].ijk.float())
                .jdata
            )
            input_semantic = batch[DS.GT_SEMANTIC][i]

            latent_semantic = semantic_from_points(
                latent_xyz, input_pc_xyz, input_semantic
            )
            latent_semantic_list.append(latent_semantic)

        batch[DS.LATENT_SEMANTIC] = latent_semantic_list
